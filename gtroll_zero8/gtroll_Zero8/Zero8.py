...
# script for the Zero8 in APC emulation mode
...
class Zero8(ControlSurface):
    __doc__ = " Script for Zero8 in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in Zero8._active_instances:
    ...
    def _do_combine(self):
        if self not in Zero8._active_instances:
            Zero8._active_instances.append(self)
            Zero8._combine_active_instances()


    def _do_uncombine(self):
        if ((self in Zero8._active_instances) and Zero8._active_instances.remove(self)):
    ...
    def _setup_session_control(self):
        is_momentary = True
        self._session = SpecialSessionComponent(8, 3)
