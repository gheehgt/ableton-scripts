...
# script for the A01 in APC emulation mode
...
class A01(ControlSurface):
    __doc__ = " A01 in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in A01._active_instances:
    ...
    def _do_combine(self):
        if self not in A01._active_instances:
            A01._active_instances.append(self)
            A01._combine_active_instances()


    def _do_uncombine(self):
        if ((self in A01._active_instances) and A01._active_instances.remove(self)):
