...
# script for the gtroll_twister in APC emulation mode
...
class gtroll_twister(ControlSurface):
    __doc__ = " Script for gtroll_twister in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in gtroll_twister._active_instances:
    ...
    def _do_combine(self):
        if self not in gtroll_twister._active_instances:
            gtroll_twister._active_instances.append(self)
            gtroll_twister._combine_active_instances()


    def _do_uncombine(self):
        if ((self in gtroll_twister._active_instances) and gtroll_twister._active_instances.remove(self)):
    ...
    def _setup_session_control(self):
        is_momentary = True
        self._session = SpecialSessionComponent(8, 1)
    ...
    def _setup_mixer_control(self):
        ...
        for track in range(8):
            ...
            strip.set_send_controls((self._ctrl_map[TRACKSENDA[track]], self._ctrl_map[TRACKSENDB[track]], self._ctrl_map[TRACKSENDC[track]], self._ctrl_map[TRACKSENDD[track]]))
